...
```bash
npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install
```
...
```text
Instalá el MCP de Google Opal: corré `npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install` y seguí lo que pida.
...
```bash
npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install --refresh
```
...
| `npm install -g` falla en Windows con execution policy | Ejecutá `cmd /c "npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install"` desde PowerShell, o abrí una terminal `cmd` directamente. |
...
| Token expirado y silent refresh no anduvo | `npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install --refresh` |

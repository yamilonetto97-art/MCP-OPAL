...
server.tool('opal_check_auth', 'Check if the Google Opal MCP server is authenticated', {}, async () => {
    ...
    try {
        ...
        if (isAuth) {
            ...
            return { content: [{ type: 'text', text: '❌ Token expired. Run "npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install --refresh" in your terminal to re-authenticate.' }] };
        }
    }
    catch (error) {
        return { content: [{ type: 'text', text: `❌ Not authenticated. Run "npx -y -p github:yamilonetto97-art/MCP-OPAL#v0.3.4 opal-mcp-install --refresh" in your terminal first to sign in to Google.` }] };
    }
});
// --- Tool: List Apps ---
server.tool('opal_list_apps', 'List all your Google Opal mini-apps', {}, async () => {
    const api = new OpalAPI();
    try {
        await api.init();
        ...
        const text = apps.map((app, i) => `${i + 1}. **${app.name}**\n   ID: ${app.id}\n   Edit: ${app.editUrl}\n   Modified: ${app.modifiedTime || 'unknown'}`).join('\n\n');
...
// --- Tool: Create App ---
server.tool('opal_create_app', 'Create a new Google Opal AI mini-app from a natural language description', { description: z.string().describe('Description of the app to create (in natural language)') }, async ({ description }) => {
    const api = new OpalAPI();
    try {
        await api.init();
...
server.tool('opal_get_app', 'Get details of a specific Opal app including its workflow nodes and connections', { appId: z.string().describe('The Opal app ID (from the URL)') }, async ({ appId }) => {
    const api = new OpalAPI();
    try {
        await api.init();
...
// --- Tool: Run App ---
server.tool('opal_run_app', 'Run/execute a Google Opal app with given input and get the output', {
...
// --- Tool: Remix App ---
server.tool('opal_remix_app', 'Remix (clone) a Google Opal gallery template into your own apps', { appId: z.string().describe('The gallery app ID to remix/clone') }, async ({ appId }) => {
    const api = new OpalAPI();
    try {
        await api.init();
...
// --- Tool: List Gallery ---
server.tool('opal_list_gallery', 'List available template apps from the Google Opal gallery', {}, async () => {
    const api = new OpalAPI();
    try {
        await api.init();
        const gallery = await api.listGallery();
        const text = gallery.map((app, i) => `${i + 1}. **${app.name}**\n   ID: ${app.id}\n   ${app.description}\n   URL: ${app.url}`).join('\n\n');
...
// --- Tool: Delete App ---
server.tool('opal_delete_app', 'Delete one of your Google Opal apps', { appId: z.string().describe('The Opal app ID to delete') }, async ({ appId }) => {
    const api = new OpalAPI();
    try {
        await api.init();

        ...
        if (resp.status === 401) {
            ...
            if (!refreshed) {
                throw new Error('Token expirado y no se pudo renovar automáticamente. ' +
                    'Corré: npx -y -p github:yamilonetto97-art/MCP-OPAL opal-mcp-install --refresh');
            }
            ...
            if (resp.status === 401) {
                throw new Error('Token sigue inválido después de renovar. ' +
                    'Corré: npx -y -p github:yamilonetto97-art/MCP-OPAL opal-mcp-install --refresh (modo interactivo)');
            }
        }
        return resp;
    }
